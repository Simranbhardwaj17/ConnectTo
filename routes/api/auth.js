  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        return res
          .status(400)
          .json({ errors: [{ msg: 'Invalid credentials' }] }); //we want to match the above err(i/p err) with existing user
      }
      ...
      jwt.sign(
        //pass payL, secret, expiration, inside callbk we will get either err or token
        payload,
        jwtSecret,
        { expiresIn: 36000 },
        (err, token) => {
